...
//int eid=(Integer)session.getAttribute("empid");
    ...
    <form method="post" onsubmit="return checkPassword(this)" action="ChangePass.vcp">
...
<%}%>
    <tr><th>Old Password:</th><br>
   <td> <input type="password" name="oldpwd" ></td></tr>
    <tr><th>New Password:</th><td><br>
    <input type="password" name="newpwd" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" 
			title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required/></td></tr>
    <tr><th>Confirm Password:</th><td><br>
    <input type="password" name="cnfpwd"></td></tr>
   <input type="hidden" name="empid" value=<%= emp.getEmployeeId()%>>
	 </table></br></br>
	 <input type="submit" value="Save"/><input type="reset" value="Refresh">
</form>
